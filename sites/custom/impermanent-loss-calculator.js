(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var rA=1+priceChangeA/100; var rB=1+priceChangeB/100; var holdValue=initialInvestment*(rA+rB)/2; var lpValue=initialInvestment*Math.sqrt(rA*rB); var impLoss=lpValue-holdValue; var impLossPct=(impLoss/holdValue)*100; var breakEvenAPR=Math.abs(impLossPct)*1;     document.getElementById('lpValue').textContent = dollar(lpValue);
    document.getElementById('holdValue').textContent = dollar(holdValue);
    document.getElementById('impLoss').textContent = dollar(impLoss);
    document.getElementById('impLossPct').textContent = fmt(impLossPct,2)+'%';
    document.getElementById('breakEvenAPR').textContent = fmt(breakEvenAPR,1)+'% APR to offset';
