(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var expr=expression.trim();var xv=parseFloat(xValue);var substituted=expr.replace(/([0-9)])x/g,'$1*x').replace(/x([0-9(])/g,'x*$1').replace(/x/g,'('+xv+')');var safe=/^[0-9+\-*/().\s]+$/.test(substituted);if(!safe){document.getElementById('answer').textContent='Error: expression contains unsupported characters';return;}try{var result=new Function('return ('+substituted+')')();document.getElementById('answer').textContent='When x = '+xv+': '+expr+' = '+fmt(result, 2);}catch(e){document.getElementById('answer').textContent='Error: could not evaluate expression';}
