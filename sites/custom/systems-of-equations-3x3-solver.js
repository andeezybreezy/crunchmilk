(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var det=a1*(b2*c3-b3*c2)-b1*(a2*c3-a3*c2)+c1*(a2*b3-a3*b2);if(Math.abs(det)<1e-10){document.getElementById('x').textContent='No unique solution';document.getElementById('y').textContent='Determinant = 0';document.getElementById('z').textContent='—';}else{var dx=d1*(b2*c3-b3*c2)-b1*(d2*c3-d3*c2)+c1*(d2*b3-d3*b2);var dy=a1*(d2*c3-d3*c2)-d1*(a2*c3-a3*c2)+c1*(a2*d3-a3*d2);var dz=a1*(b2*d3-b3*d2)-b1*(a2*d3-a3*d2)+d1*(a2*b3-a3*b2);document.getElementById('x').textContent=fmt(dx/det,4);document.getElementById('y').textContent=fmt(dy/det,4);document.getElementById('z').textContent=fmt(dz/det,4);}
