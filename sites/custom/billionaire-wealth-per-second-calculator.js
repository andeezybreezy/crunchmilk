(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var wealths = {musk: 240, bezos: 200, zuckerberg: 180, gates: 130, buffett: 130, ellison: 145, custom: customWealth}; var wealth = wealths[billionaire] * 1e9; var annualGain = wealth * 0.15; var perSecond = annualGain / (365.25 * 24 * 3600); var perMinute = perSecond * 60; var yourPerSecond = yourIncome / (365.25 * 24 * 3600); var ratio = Math.round(perSecond / yourPerSecond); var timeForSalary = yourIncome / perSecond; var timeStr = timeForSalary < 60 ? Math.round(timeForSalary) + ' seconds' : timeForSalary < 3600 ? (timeForSalary / 60).toFixed(1) + ' minutes' : (timeForSalary / 3600).toFixed(1) + ' hours'; var yearsToWealth = Math.round(wealth / yourIncome); var yearsStr = yearsToWealth > 1e6 ? (yearsToWealth / 1e6).toFixed(1) + ' million years' : yearsToWealth.toLocaleString() + ' years';     document.getElementById('wealthPerSecond').textContent = '$' + Math.round(perSecond).toLocaleString() + '/sec';
