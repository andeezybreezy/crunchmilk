(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var pricing={haiku:{input:0.25,output:1.25},sonnet:{input:3,output:15},opus:{input:15,output:75}}; var p=pricing[model]; var inCost=(inputTokens/1000000)*p.input; var outCost=(outputTokens/1000000)*p.output; var costPerReq=inCost+outCost; var dailyCost=costPerReq*requests; var monthlyCost=dailyCost*days; var monthlyIn=inCost*requests*days; var monthlyOut=outCost*requests*days;     document.getElementById('costPerReq').textContent = '$'+fmt(costPerReq, 2);
