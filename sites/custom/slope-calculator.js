(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var rise=y2-y1;var run=x2-x1;if(run===0){document.getElementById('slope').textContent='Undefined';document.getElementById('slopeType').textContent='Vertical Line (Undefined)';document.getElementById('riseRun').textContent=rise+' / 0';}else{var m=rise/run;document.getElementById('slope').textContent=fmt(m, 2);document.getElementById('slopeType').textContent=m>0?'Positive (rises left to right)':m<0?'Negative (falls left to right)':m===0?'Zero (horizontal line)':'Zero';document.getElementById('riseRun').textContent=rise+' / '+run;}
