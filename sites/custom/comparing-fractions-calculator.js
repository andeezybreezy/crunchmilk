(function() {
  ...
  function calculate() {
    ...
    // Calculation logic
    var n1=parseInt(num1),d1=parseInt(den1),n2=parseInt(num2),d2=parseInt(den2);if(d1===0||d2===0){document.getElementById('comparison').textContent='Error: denominator cannot be zero';return;}var dec1=n1/d1,dec2=n2/d2;document.getElementById('decimal1').textContent=fmt(dec1, 2);document.getElementById('decimal2').textContent=fmt(dec2, 2);var cross1=n1*d2,cross2=n2*d1;if(cross1>cross2){document.getElementById('comparison').textContent=n1+'/'+d1+' > '+n2+'/'+d2+' (first fraction is greater)';}else if(cross1<cross2){document.getElementById('comparison').textContent=n1+'/'+d1+' < '+n2+'/'+d2+' (second fraction is greater)';}else{document.getElementById('comparison').textContent=n1+'/'+d1+' = '+n2+'/'+d2+' (the fractions are equal)';}
